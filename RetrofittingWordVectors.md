...
这种先验可以通过**最大后验(MAP)估计**在学习过程中被纳入。由于不存在估计的闭解，我们考虑两个迭代过程。首先，我们使用对数似然(由现有向量学习模型给出)和对数先验(由Eq. 2)对Q的梯度和进行学习。由于计算Eq. 2的梯度在词汇量为n的情况下具有线性运行时间，所以我们在训练期间对每k个单词使用延迟更新(Carpenter, 2008)。我们称之为MAP的惰性方法。第二种方法是对对数似然进行随机梯度上升，每k个单词之后对Eq. 1进行更新。我们称之为周期法。我们稍后将这些方法与改造方法进行实验比较(6.2)。
...
释义数据库(Ganitkevitch et al.， 2013)是一个语义词典，包含超过2.2亿对英语释义。其中，800万是词汇(单字对单字)释义。词汇释义习得背后的关键直觉是，一种语言中的两个词在平行文本中与另一种语言中的同一个词对齐，应该是同义词。例如，如果“监禁”和“监禁”两个词在另一种语言中被翻译为同一个词，则可以合理地假定它们具有相同的意思。在我们的实验中，我们为PPDB中的每个词汇释义实例化了E中的一个边。词汇释义数据集大小不一，从S到XXXL，释义置信度依次降低，大小依次增加。我们选择了XL来做实验。我们想给更高的边的权值(α)连接翻新词向量(q)纯粹的分配向量(q)比边缘连接相互改进的向量(βij),所以我将所有α1和βij学位(i) 1(我是节点更新被应用于)。
...
我们使用它们的联合模型训练代码8训练单词向量，同时使用它们的最佳模型中指定的完全相同的训练设置:CBOW，向量长度100和PPDB进行富集。结果显示在表4的上半部分，我们的模型始终优于基线和他们的模型。

<h4>Xu et al.(2014)</h4>

该模型从Freebase9中提取单词之间的分类和关系知识，并将其作为训练时的约束。不幸的是，他们的单词嵌入和模型训练代码都不是公开可用的，所以我们使用完全相同的设置来训练SG模型(向量长度300)，并且使用同一个语料库:单语英语维基百科文本我们将我们的改进向量在SYN-REL和WS353任务上的性能与他们论文中报告的最佳模型进行了比较。如表4下半部分所示，我们的模型在SYN-REL任务上比他们的模型高出5.5分，但在WS-353任务上略低。

<h4>6.4 多种验证</h4>

我们在另外三种语言上测试了我们的方法:德语、法语和西班牙语。我们使用Universal WordNet (de Melo and Weikum, 2009)，这是一个基于WordNet自动构建的多语言词汇知识库它包含了通过不同的词汇关系连接到其他单词的单词，包括语言内部和跨语言。我们为不同的语言构建单独的图表(即，在同一语言中仅将单词与其他单词连接起来)，并对每一种语言进行改进。由于英语以外的语言没有多少词语相似度评估基准，我们测试了我们的基准，并在每种语言的一个基准上改进了向量。

我们分别在德语、法语和西班牙语中使用了RG-65 (Gurevych, 2005)、RG-65 (Joubarne和Inkpen, 2011)和MC-30 (Hassan和Mihalcea, 2009)我们在从维基百科中提取的10亿个标记语料库中训练长度为300的每种语言的SG向量，并在改进前后的基准上根据单词相似度评估它们。表5显示我们取得了很大的改进，这强烈地表明我们的方法适用于这些语言。

<h3>7 进一步的分析</h3>

<h3>改进 vs 向量长度</h3>

有了更多的维度，单词向量可能能够捕获更高阶的语义信息，而修改可能没有多大帮助。我们在10亿个英语标记上训练SG向量，向量长度从50到1000不等，并在MEN单词相似度任务上进行评估。我们将这些载体改造成PPDB(§4)，并对相同任务的载体进行评估。图图2显示了不同长度的向量质量的一致改善。

![1631699758(1)](PaperImges\9-13Retrofitting\1631699758(1).jpg)

<center>图3：保持形容词和副词关系的100维SG向量对的二维PCA投影，在(左)和(右)改造之前</center>

![1631699842(1)](PaperImges\9-13Retrofitting\1631699842(1).jpg)

<center>表 5：利用原始和改进的SG向量进行词相似度评价的斯皮尔曼相关</center>

![1631699896(1)](PaperImges\9-13Retrofitting\1631699896(1).jpg)

<center>图2：改进前后MEN词相似度任务的Spearman相关性。</center>

<h4>可视化</h4>

我们从SYN-REL任务(5)中随机选择8对形容词-副词关系的词对，然后对100维SG词向量进行二维主成分分析投影，并在r2中绘制它们。在图3中，我们在改造前(左)和改造后(右)绘制了这些投影。可以看出，在第一种情况下，类比向量的方向是不一致的，但经过改造后，所有的类比向量都朝着同一个方向排列。

<h3> 8 相关工作</h3>

词汇语义信息在词汇向量训练中的应用受到限制。最近，单词相似度知识(Yu and Dredze, 2014;Fried and Duh, 2014)和词关系知识(Xu et al.， 2014;Bian等人，2014)已经被用于改进word2vec嵌入的联合训练模型，类似于我们的正则化方法。在潜在语义分析中，可以构造词的共现矩阵来结合关系信息，如反义词特定极性感应(Yih et al.， 2012)和多关系潜在语义分析(Chang et al.， 2013)。

我们提出的方法在概念上类似于以前的工作，即使用图结构在语义概念之间传播信息(Zhu, 2005;Culp and Michailidis, 2008)。基于图的信念传播也被用于诱导POS标签(Subramanya et al.， 2010;Das和Petrov, 2011)和语义框架关联(Das和Smith, 2011)。在这些努力中，未知单词的标签是用一种与我们相似的方法推断出来的。广义上，基于图的半监督学习(Zhu, 2005;Talukdar和Pereira, 2010)已被应用于机器翻译(Alexandrescu和Kirchhoff, 2009)、无监督语义角色归纳(Lang和Lapata, 2011)、语义文档建模(schuhacher和Ponzetto, 2014)、语言生成(Krahmer et al.， 2003)和情感分析(Goldberg和Zhu, 2006)。

<h3>9 结论</h3>

本文提出了一种利用语义词典中的词关系知识改进词向量的简单有效方法——修正法。改进被用作一个后处理步骤，以提高矢量质量，并且比其他在训练时使用语义信息的方法更模块化。它可以应用于任何单词向量训练方法得到的向量。我们的实验探索了该方法在任务、语义词汇和语言中的性能，并表明它优于现有的替代方法。改装工具可在（链接）
